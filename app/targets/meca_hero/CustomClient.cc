    ...
    if (rx_data[0] == CClientSOF && rx_data[Len - 1] == CClientEOF) {
      switch (rx_data[1]) {
        case CClient_USB_Rx_ID:
          ...
          _mouse_x = static_cast<int16_t>(CClient_Rx.payload[0] | CClient_Rx.payload[1] << 8);
          _mouse_y = static_cast<int16_t>(CClient_Rx.payload[2] | CClient_Rx.payload[3] << 8);
          _mouse_z = static_cast<int16_t>(CClient_Rx.payload[4] | CClient_Rx.payload[5] << 8);
