...
static void show_limit_warning(void) {
  ...
  // 注：若默认字体未开启中文字库，中文会乱码。这里暂时使用英文替代"不可超出限度"。如果有中文字库可用，你可以将其改回
  // "不可超出限度"
...
static void btn_edit_confirm_cb(lv_event_t *e) {
  if (current_category == 0)
    Pitch[current_index] = temp_val;
  else
    Yaw[current_index] = temp_val;
...
extern "C" void init_lvgl_demo(void) {
  ...
  lv_disp_set_theme(NULL, lv_theme_default_init(NULL, lv_palette_main(LV_PALETTE_BLUE), lv_palette_main(LV_PALETTE_RED),
