...
void NavigateCanCommunicator::RxCallback(const hal::CanFrame *msg) {
  if (msg->rx_std_id == 0x190) {
    ReportStatus(kOk);
    chassis_target_x_ =  static_cast<f32>(static_cast<u16>(msg->data[0]) << 8 | static_cast<u16>(msg->data[1])) / 10000.0f;
    chassis_target_y_ = static_cast<f32>(static_cast<u16>(msg->data[2]) << 8 | static_cast<u16>(msg->data[3])) / 10000.0f;
    chassis_target_w_ = static_cast<f32>(static_cast<u16>(msg->data[4]) << 8 | static_cast<u16>(msg->data[5])) / 10000.0f;
    target_yaw_speed_ = static_cast<f32>(msg->data[6]) / 100.0f;
    scan_mode_ = static_cast<u8>(msg->data[7]);
  }
}

void NavigateCanCommunicator::Update() {
  this->can_->Write(0x180, tx_buf_, 8);
}
