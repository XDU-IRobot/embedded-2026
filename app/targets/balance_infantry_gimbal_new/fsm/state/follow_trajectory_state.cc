...
etl::fsm_state_id_t FollowTrajectory::on_event(const event::ControlLoop &e) {
  ...
  //     const auto ff = dynamics_.ComputeFf(yaw_target_, pitch_target_, yaw_traj.dq * 2.5f, pitch_traj.dq*0.8,
  //     yaw_traj.ddq * 2.5f, pitch_traj.ddq*0.8, {0, 0, -9.81});
  const auto ff = globals->dynamics_.ComputeFf(yaw_target_, pitch_target_, yaw_traj.dq * 2.5f, pitch_traj.dq * 0.8,
                                               yaw_traj.ddq * 2.5f, pitch_traj.ddq * 0.8, {0, 0, -9.81});
  yaw_ff_ = ff(0) * 1.1;
  pitch_ff_ = ff(1);
  globals->gimbal_controller.SetTarget(yaw_target_, pitch_target_);
  globals->gimbal_controller.Update(globals->yaw_motor.pos(), globals->yaw_motor.vel(), globals->pitch_motor.pos(),
...
etl::fsm_state_id_t FollowTrajectory::on_event(const event::ForceModeSwitch &e) { return e.target_mode; }

etl::fsm_state_id_t FollowTrajectory::on_event_unknown(const etl::imessage &) { return No_State_Change; }
}  // namespace state
}  // namespace fsm
