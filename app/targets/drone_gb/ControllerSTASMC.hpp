...
class GimbalSTASMC {
 public:
  ...
  void Update(float yaw_position, float yaw_speed, float pitch_position, float pitch_speed, float dt = 0.001f) {
    ...
    float pitch_accel = clamp(last_.pitch_filtered_accel, -params_.pitch_model.max_accel, params_.pitch_model.max_accel);

    // 4. 计算基于物理模型的前馈输出 (J*a + B*v + 外部扭矩前馈)
    float yaw_model_ff = params_.yaw_model.J * yaw_accel + params_.yaw_model.B * yaw_target_speed + target_.yaw_torque_ff;
    float pitch_model_ff = params_.pitch_model.J * pitch_accel + params_.pitch_model.B * pitch_target_speed + target_.pitch_torque_ff;
    ...
    output_.yaw = params_.yaw_spd.kp * yaw_s +                                  
                  params_.yaw_spd.k1 * std::sqrt(std::fabs(yaw_s)) * yaw_sat +  
                  yaw_i_ +                                                      
                  yaw_model_ff;                                        

    output_.pitch = params_.pitch_spd.kp * pitch_s + 
