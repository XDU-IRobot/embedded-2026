...
| 区域 | 初始化内容 | 动态更新 |
| --- | --- | --- |
| 中央/左右下方（`UIInfantry`） | 中心十字、超级电容能量进度条及高速模式 `ON/OFF`、瞄速调整数值、自瞄文本 `GETTARGET/SUGGESTFIRE`、模式文字 `F R N U D X / H N S`、状态指示矩形 | 超电能量（低于 40% 为粉色，否则绿色）、实际高速模式开关状态、自瞄得到目标/建议开火、跟随/旋转/无力状态、大小符、速度模式及低 buffer 指示 |
...
`UiRefresh()` 对 `UI_show_flag` 做上升沿检测，并且只在机器人 ID 有效时调用一次 `static_UI_add()`。云台板需要先将该标志清零，再重新置一，才能请求客户端重新创建整套 UI；标志持续为 1 不会重复发送 Add 图元，静态任务发送完成后由动态 Edit 任务持续刷新进度条、`ON/OFF` 和其他状态。
