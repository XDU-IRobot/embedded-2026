...
void Gimbal_Solver_WithRoll::Update(float imu_yaw, float imu_pitch, float imu_roll, float motor_yaw_angle,
  ...
  G_in_W_ << cos(imu_yaw_) * cos(imu_pitch_),
      cos(imu_yaw_) * sin(imu_pitch_) * sin(imu_roll_) - sin(imu_yaw_) * cos(imu_roll_),
      cos(imu_yaw_) * sin(imu_pitch_) * cos(imu_roll_) + sin(imu_yaw_) * sin(imu_roll_),
      sin(imu_yaw_) * cos(imu_pitch_),
      sin(imu_yaw_) * sin(imu_pitch_) * sin(imu_roll_) + cos(imu_yaw_) * cos(imu_roll_),
      sin(imu_yaw_) * sin(imu_pitch_) * cos(imu_roll_) - cos(imu_yaw_) * sin(imu_roll_), -sin(imu_pitch_),
      cos(imu_pitch_) * sin(imu_roll_), cos(imu_pitch_) * cos(imu_roll_);

  T_in_W_ << cos(imu_target_pitch_) * cos(imu_target_yaw_), cos(imu_target_pitch_) * sin(imu_target_yaw_),
      -sin(imu_target_pitch_);

  G_in_B_ << cos(base_yaw_angle_) * cos(base_pitch_angle_), -sin(base_yaw_angle_),
      cos(base_yaw_angle_) * sin(base_pitch_angle_), sin(base_yaw_angle_) * cos(base_pitch_angle_),
      cos(base_yaw_angle_), sin(base_yaw_angle_) * sin(base_pitch_angle_), -sin(base_pitch_angle_), 0,
      cos(base_pitch_angle_);
 ...
 */
float Gimbal_Solver_WithRoll::Get_Motor_Target_Pitch() { return motor_target_pitch_; }
 ...
 * @returns        基座系下的目标角度，后续pid将使用该角度作为set，电机反馈值作为ref
 */
float Gimbal_Solver_WithRoll::Get_Motor_Target_Yaw() { return motor_target_yaw_; }
 ...
 */
float Gimbal_Solver_WithRoll::Get_Base_Yaw_Angle() { return base_yaw_angle_; }
 ...
 * @returns        可调试确认零点和方向是否正确
 */
float Gimbal_Solver_WithRoll::Get_Base_Pitch_Angle() { return base_pitch_angle_; }
