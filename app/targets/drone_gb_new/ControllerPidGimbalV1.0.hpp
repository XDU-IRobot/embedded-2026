...
class Gimbal2DofV1 {
 public:
  ...
  Gimbal2Dof() {
    ...
    pid_.pitch_position.SetFuzzy(true).SetFuzzyErrorScale(M_PI).SetFuzzyDErrorScale(M_PI * 100);
  }
 ...
 private:
  ...
  void UpdatePositionLoop(float dt) {
    ...
    pid_.yaw_position.Update(target_.yaw_position, state_.yaw_position, position_dt);

    pid_.pitch_position.Update(target_.pitch_position, state_.pitch_position, position_dt);
