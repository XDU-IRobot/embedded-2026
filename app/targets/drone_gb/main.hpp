...
public:
    ...
    rm::modules::BuzzerController<rm::modules::buzzer_melody::Silent,
                                  ...
                                  rm::modules::buzzer_melody::SeeUAgain,
    ...
    void GimbalInit()
    {
        ...
        yaw_motor = new rm::device::GM6020{*can1, 5};
    ...
    void GimbalPIDInit()
    {
        gimbal_controller.pid().yaw_position.SetKp(5.0f);  // 位置环 0.25f 0.0f 0.025f
        gimbal_controller.pid().yaw_position.SetKi(0.0f);
        gimbal_controller.pid().yaw_position.SetKd(0.0f);
        gimbal_controller.pid().yaw_position.SetMaxOut(2000.0f);
        gimbal_controller.pid().yaw_position.SetMaxIout(1000.0f);
        gimbal_controller.pid().yaw_speed.SetKp(5.0f);  // 速度环 520.0f 0.0f 50.0f
        gimbal_controller.pid().yaw_speed.SetKi(0.0f);
        gimbal_controller.pid().yaw_speed.SetKd(0.0f);
        gimbal_controller.pid().yaw_speed.SetMaxOut(2000.0f);
        gimbal_controller.pid().yaw_speed.SetMaxIout(1000.0f);

        gimbal_controller.pid().pitch_position.SetKp(0.1f);  // 位置环 15.0f 0.0f 1.0f
        ...
        gimbal_controller.pid().pitch_speed.SetKp(0.1f);  // 速度环 1.6f 0.0f 4.5f
    ...
    void GimbalControl()
    {
        ...
        if (GimbalState_==kManual)
        {
            ...
            gimbal_controller.SetTarget(rc_yaw_data,rc_yaw_data,0,0);
            gimbal_controller.Update(yaw,yaw_motor->rpm(),pitch,pitch_motor->vel());
        }
    ...
    void SubLoop500Hz()
    {
        ...
        imu->Update();
        ahrs.Update(rm::modules::ImuData6Dof{
             -imu->gyro_x(),  -imu->gyro_y(),  imu->gyro_z(),
             -imu->accel_x(),  -imu->accel_y(),  imu->accel_z()});
        yaw = -57.3 * ahrs.euler_angle().roll + 180;  // 云台正向靠近Yaw的0°和360°边界，顺时针从0°增大到360°
        roll = 57.3 * ahrs.euler_angle().yaw + 180;  // 云台始终处于Roll的0°和360°边界，顺时针从0°增大到360°(不限位)
