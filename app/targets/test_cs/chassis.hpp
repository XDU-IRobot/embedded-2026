...
class Chassis {
 ...
 public:
  ...
  void UpdateTargetSpeed(float x, float y, float w) {
    ...
    switch (phase) {
      case 0:
        motors_.lf->MotorVelCtrl(calc_dir_lf(wheel_speeds.lf_speed), RadsToRpm(wheel_speeds.lf_speed), 0, true);
        break;

      case 1:
        motors_.lb->MotorVelCtrl(calc_dir_lb(wheel_speeds.lr_speed), RadsToRpm(wheel_speeds.lr_speed), 0, true);
        break;

      case 2:
        motors_.rf->MotorVelCtrl(calc_dir_rf(wheel_speeds.rf_speed), RadsToRpm(wheel_speeds.rf_speed), 0, true);
        break;

      case 3:
        motors_.rb->MotorVelCtrl(calc_dir_rb(wheel_speeds.rr_speed), RadsToRpm(wheel_speeds.rr_speed), 0, true);
