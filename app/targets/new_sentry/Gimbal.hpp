...
inline class Gimbal {
 public:
  ...
  const f32 mid_up_yaw_pos_ = 5870.0f;  // 云台上部yaw轴中值（编码器值）
 ...
 private:
  ...
  const f32 sensitivity_up_yaw_ = 0.2f;      // 云台上部yaw轴灵敏度
  ...
  const u16 max_up_yaw_pos_ = 6960;     // 云台上部yaw轴最大（编码器值）
  const u16 min_up_yaw_pos_ = 4780;     // 云台上部yaw轴最小（编码器值）
