...
/**
 * @brief 二轴云台 级联双环 超螺旋滑模控制器 (位置外环 + 速度内环 STA)
 */
class Gimbal2DofSMC {
 public:
  ...
  void Update(float yaw_position, float yaw_speed,
    ...
    // ================== 外环：位置环 (P 控制) ==================
    // 计算位置误差
    const float yaw_pos_err = target_.yaw_position - state_.yaw_position;
    const float pitch_pos_err = target_.pitch_position - state_.pitch_position;
