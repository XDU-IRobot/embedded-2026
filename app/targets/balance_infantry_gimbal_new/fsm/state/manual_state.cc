...
etl::fsm_state_id_t Manual::on_event(const event::ControlLoop &e) {
  rc_yaw_tar -= rm::modules::Map((globals->rc.left_x() < 10 && globals->rc.left_x() > -10) ? 0 : globals->rc.left_x(),
                                 -660, 660, -0.005f, 0.005f);
  ...
  globals->pitch_motor.SetMitCommand(
      0, 0, globals->gimbal_controller.output().pitch + 1.3 * cos(globals->ahrs.euler_angle().pitch), 0, 0);

  return No_State_Change;
};
etl::fsm_state_id_t Manual::on_event(const event::ForceModeSwitch &e) { return e.target_mode; }

etl::fsm_state_id_t Manual::on_event_unknown(const etl::imessage &) { return No_State_Change; }
}  // namespace state
}  // namespace fsm
