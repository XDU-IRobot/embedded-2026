...
void GimbalStateUpdate()
{
    static bool aimbot_on = (globals->aimbot_comm.aimbot_state() >> 0 & 0x01 || globals->aimbot_comm.aimbot_state() >> 0 & 0x03);
...
//    }
    ...
    globals->aimbot_comm.UpdateControl(globals->ahrs.euler_angle().yaw, globals->ahrs.euler_angle().pitch,
                                       globals->ahrs.euler_angle().roll, 3, 1, globals->imu_count,
                                       0);
}
...
{
    if (globals->rc.online_status() == rm::device::Device::kOk) {
        if (globals->rc.switch_l() == rm::device::DR16::SwitchPosition::kUp ||globals->rc.switch_r() == rm::device::DR16::SwitchPosition::kDown) {
