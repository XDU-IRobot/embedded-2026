...
class Gimbal2Dof {
 public:
  Gimbal2Dof() {
    // 位置环参数
    pid_.yaw_position.SetCircular(true)
        .SetCircularCycle(M_PI * 2)
        .SetFuzzy(true)
        .SetFuzzyErrorScale(M_PI)
        .SetFuzzyDErrorScale(M_PI * 100);
    pid_.pitch_position.SetFuzzy(true).SetFuzzyErrorScale(M_PI);
    // 速度环与电流环可在此按需初始化
  }
   ...
   */
  void Update(float yaw_position, float yaw_speed, float yaw_current, float pitch_position, float pitch_speed,
              float pitch_current, float dt = 1.f) {
  ...
  void SetTarget(float yaw_position, float pitch_position, float yaw_speed_feedforward = 0.f,
                 float yaw_current_feedforward = 0.f, float pitch_current_feedforward = 0.f) {
