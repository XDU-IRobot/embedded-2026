...
void MainLoop() {
  ...
  globals->rc_yaw_data += Map(globals->rc->left_x(), -660, 660, -0.3f, 0.3f);  // 遥控器映射yaw轴
  globals->rc_yaw_data = LoopConstrain(globals->rc_yaw_data, 0, 360);
  globals->rc_pitch_data += Map(globals->rc->left_y(), -660, 660, -0.3f, 0.3f);  // 遥控器映射pitch轴
  globals->rc_pitch_data = Constrain(globals->rc_pitch_data, 150.0f, 190.0f);

  globals->gimbal_controller.SetTarget(globals->rc_yaw_data, globals->rc_pitch_data);
  globals->gimbal_controller.Update(
      -57.3 * -globals->ahrs.euler_angle().roll + 180, globals->yaw_motor->rpm() * (2.f * M_PI / 60.f),
      -57.3 * globals->ahrs.euler_angle().pitch + 180, globals->pitch_motor->vel() * (2.f * M_PI / 60.f));
      ...
      etl::delegate<void(const rm::device::DR16::SwitchPosition &, const rm::device::DR16::SwitchPosition &)>::create(
          [&](const rm::device::DR16::SwitchPosition &old_value, const rm::device::DR16::SwitchPosition &new_value) {
            if (new_value == rm::device::DR16::SwitchPosition::kUp ||
                new_value == rm::device::DR16::SwitchPosition::kMid) {
