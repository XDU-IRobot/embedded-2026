...
void Chassis::ChassisStateUpdate() {
    ...
    if (!globals->device_chassis.all_device_ok()) { chassis->ChassisDisableUpdate(); }
}
...
void Chassis::ChassisRCDataUpdate() {
    chassis->down_yaw_delta_ =
            ...
            rm::modules::Map(gimbal->mid_up_yaw_angle_ - static_cast<f32>(globals->up_yaw_motor->encoder()),
    ...
    chassis->chassis_receive_x_ = rm::modules::Map(globals->rc->right_y(),
                                                   -globals->rc_max_value_, globals->rc_max_value_,
                                                   -chassis->chassis_sensitivity_xy_, chassis->chassis_sensitivity_xy_);
    chassis->chassis_receive_y_ = rm::modules::Map(globals->rc->right_x(),
                                                   -globals->rc_max_value_, globals->rc_max_value_,
                                                   -chassis->chassis_sensitivity_xy_, chassis->chassis_sensitivity_xy_);
    ...
    if (globals->rc->dial() >= 650) {
        chassis->chassis_target_w_ = 8000.0f;
    } else {
    ...
    chassis->chassis_target_x_ = rm::modules::Clamp(chassis->chassis_target_x_,
                                                    -chassis->chassis_max_speed_xy_, chassis->chassis_max_speed_xy_);
    chassis->chassis_target_y_ = rm::modules::Clamp(chassis->chassis_target_y_,
                                                    -chassis->chassis_max_speed_xy_, chassis->chassis_max_speed_xy_);
    chassis->chassis_target_w_ = rm::modules::Clamp(chassis->chassis_target_w_,
...
void Chassis::ChassisMovePIDUpdate() {
    ...
    steer_delta_angle_lf = rm::modules::Map(steer_delta_angle_lf, 0.0f, globals->GM6020_encoder_max_,
                                            0.0f, 2.0f * static_cast<f32>(M_PI));
    steer_delta_angle_rf = rm::modules::Map(steer_delta_angle_rf, 0.0f, globals->GM6020_encoder_max_,
                                            0.0f, 2.0f * static_cast<f32>(M_PI));
    steer_delta_angle_lb = rm::modules::Map(steer_delta_angle_lb, 0.0f, globals->GM6020_encoder_max_,
                                            0.0f, 2.0f * static_cast<f32>(M_PI));
    steer_delta_angle_rb = rm::modules::Map(steer_delta_angle_rb, 0.0f, globals->GM6020_encoder_max_,
                                            0.0f, 2.0f * static_cast<f32>(M_PI));
...
void Chassis::ChassisNavigateDataUpdate() {
    ...
    chassis->down_yaw_delta_ = rm::modules::Wrap(chassis->down_yaw_delta_, -static_cast<f32>(M_PI), M_PI);
    chassis->chassis_target_x_ =
            rm::modules::Map(globals->NucControl->vx, -chassis->chassis_max_navigate_xy_,
                             chassis->chassis_max_navigate_xy_, -chassis->chassis_sensitivity_xy_,
                             chassis->chassis_sensitivity_xy_) * std::cos(chassis->down_yaw_delta_) -
            rm::modules::Map(globals->NucControl->vy, -chassis->chassis_max_navigate_xy_,
                             chassis->chassis_max_navigate_xy_, -chassis->chassis_sensitivity_xy_,
                             chassis->chassis_sensitivity_xy_) * std::sin(chassis->down_yaw_delta_);
    chassis->chassis_target_y_ =
            rm::modules::Map(globals->NucControl->vy, -chassis->chassis_max_navigate_xy_,
                             chassis->chassis_max_navigate_xy_, -chassis->chassis_sensitivity_xy_,
                             chassis->chassis_sensitivity_xy_) * std::cos(chassis->down_yaw_delta_) +
    ...
    chassis->chassis_target_w_ = rm::modules::Clamp(chassis->chassis_target_w_,
                                                    -chassis->chassis_max_speed_w_, chassis->chassis_max_speed_w_);
    chassis->chassis_target_x_ = 0.0f;
    chassis->chassis_target_y_ = 0.0f;
    chassis->chassis_target_w_ = 0.0f;
}
