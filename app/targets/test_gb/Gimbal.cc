...
/***********************************************************************************************
  ...
  // }
  //
  // globals->gimbal_controller.output().yaw = modules::Wrap(J * (C * (d_target - d_real) + dd_target + epson * sat + K * s) + B * d_real,-6,6);
  //globals->gimbal_controller.output().yaw = modules::Wrap(J * (C * (d_target - d_real) + dd_target + k1 * pow(abs(s),0.5)*sign(s)+k2*i_sign_s) + B * d_real,-6,6);
***********************************************************************************************************************/
...
/***************************************************************************************************************/
  //前馈，roll轴补偿
  globals->gimbal_solver_with_roll->Update(globals->ahrs.euler_angle().yaw,globals->ahrs.euler_angle().pitch,globals->ahrs.euler_angle().roll
                                          ,globals->yaw_motor->pos(),globals->pitch_motor->pos()
                                          ,gimbal_yaw_target_,gimbal_pitch_target_);
  ...
  globals->gimbal_controller.Update(globals->gimbal_solver_with_roll->Get_Base_Yaw_Angle(), globals->yaw_motor->vel(),
                                    globals->gimbal_solver_with_roll->Get_Base_Pitch_Angle(), globals->pitch_motor->vel());
